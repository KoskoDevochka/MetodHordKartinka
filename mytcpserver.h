 ...
 * - Административные команды (задание "функции для админа")
 */
...
private:
    ...
    struct ClientInfo {
        ...
        bool authenticated;   ///< Авторизован ли клиент
        bool isAdmin;         ///< Является ли администратором
    };
     ...
     * \param request Строка запроса (например "action1 Hello" или "0|текст")
     */
    ...
    void handleListLogs(QTcpSocket* client);
    void handleClearLogs(QTcpSocket* client);
    void handleDeleteUser(QTcpSocket* client, const QString& username);

    // ОБРАБОТКА ОСНОВНЫХ ФУНКЦИЙ (Виженер, SHA, хорды, стеганография)
    void handleFunctionCall(QTcpSocket* client, int funcId, const QString& inputData);
